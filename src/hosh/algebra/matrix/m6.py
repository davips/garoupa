...
from hosh.algebra.matrix.mat6 import Mat6
...
class M6:
    def __post_init__(self):
        self.order = 2 ** 15
        self.sorted = lambda: (Mat6(i) for i in range(self.order))
        self.id = Mat6(0)
        self.bits = 15

    def __iter__(self):
        for i in range(self.order):
            yield Mat6(rnd.getrandbits(self.bits))
    ...
    def __invert__(self):
        return Mat6(rnd.getrandbits(self.bits))
